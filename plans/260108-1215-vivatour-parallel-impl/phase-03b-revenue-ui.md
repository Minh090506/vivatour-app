...
Add Revenue UI components for 3-tier lock system including history panel, lock dialog, and table updates.

---

## Task 3B.1: Revenue History Panel (15 min)
...
## Task 3B.2: Revenue Lock Dialog (10 min)
...
## Task 3B.3: Update Revenue Table (15 min)

### File: `src/components/revenues/revenue-table.tsx`

Update lock column and actions for 3-tier system.
...
// Update lock/unlock action buttons in row actions:
function RowActions({ row, onRefresh }: { row: Revenue; onRefresh: () => void }) {
  const [lockDialogOpen, setLockDialogOpen] = useState(false);
  const [historyOpen, setHistoryOpen] = useState(false);
  const { data: session } = useSession();

  const canLock = session?.user?.role === 'ACCOUNTANT' || session?.user?.role === 'ADMIN';
  const canUnlock = session?.user?.role === 'ADMIN';

  // Determine next tier that can be unlocked (reverse order)
  const nextUnlockTier: LockTier | null = row.lockFinal
    ? 'Final'
    : row.lockAdmin
    ? 'Admin'
    : row.lockKT
  ...
  const handleUnlock = async () => {
    ...
    try {
      const res = await fetch(`/api/revenues/${row.id}/unlock`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(`Đã mở khóa ${LOCK_TIER_LABELS[nextUnlockTier]}`);
      onRefresh();
    } catch (error) {
  ...
  const isLocked = row.lockKT || row.lockAdmin || row.lockFinal;
  const isFullyLocked = row.lockFinal;

  // Determine if more tiers can be locked
  const canLockMore = !row.lockFinal;
...
## Task 3B.4: Update Revenue List Page (10 min)

### File: `src/app/(dashboard)/revenues/page.tsx`

Update filter options for lock status.

```tsx
// Update lock status filter options
const lockStatusOptions = [
  { value: 'all', label: 'Tất cả' },
  { value: 'unlocked', label: 'Chưa khóa' },
  { value: 'lockKT', label: 'Đã khóa KT' },
  { value: 'lockAdmin', label: 'Đã khóa Admin' },
  { value: 'lockFinal', label: 'Đã khóa Cuối' },
];

// Update filter logic
const filterRevenues = (revenues: Revenue[], filters: Filters) => {
  return revenues.filter((r) => {
    // ... other filters ...

    // Lock status filter
    if (filters.lockStatus && filters.lockStatus !== 'all') {
      if (filters.lockStatus === 'unlocked') {
        if (r.lockKT || r.lockAdmin || r.lockFinal) return false;
      } else if (filters.lockStatus === 'lockKT') {
        if (!r.lockKT) return false;
      } else if (filters.lockStatus === 'lockAdmin') {
        if (!r.lockAdmin) return false;
      } else if (filters.lockStatus === 'lockFinal') {
        if (!r.lockFinal) return false;
      }
    }
...
- [ ] Row actions include lock/unlock per tier
- [ ] History sheet opens from row actions
- [ ] Filter options include tier-specific status
...
Add to `src/components/revenues/index.ts` (if exists):
