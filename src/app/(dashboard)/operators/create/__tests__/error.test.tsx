/**
 * Tests for create operator error boundary
 * Covers: error catching, Vietnamese messages, retry/back buttons
 */

import { render, screen, fireEvent } from '@testing-library/react';
import CreateOperatorError from '../error';
...
describe('CreateOperatorError', () => {
  ...
  describe('Error Boundary Behavior', () => {
    it('renders error fallback UI when error is caught', () => {
      render(<CreateOperatorError error={mockError} reset={mockReset} />);

      expect(screen.getByText('Lỗi tạo dịch vụ')).toBeInTheDocument();
      expect(
        screen.getByText('Không thể tải trang tạo dịch vụ. Vui lòng thử lại hoặc quay lại danh sách.')
      ).toBeInTheDocument();
    });
    ...
    it('logs error to console', () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith('[CreateOperatorError]', mockError);
    });
  });
  ...
  describe('Error Display', () => {
    ...
    it('handles error with digest property', () => {
      ...
      errorWithDigest.digest = 'create-xyz789';

      render(<CreateOperatorError error={errorWithDigest} reset={mockReset} />);

      expect(mockConsoleError).toHaveBeenCalledWith('[CreateOperatorError]', errorWithDigest);
    });
  });

  describe('Layout', () => {
    it('centers content in container', () => {
      const { container } = render(<CreateOperatorError error={mockError} reset={mockReset} />);
