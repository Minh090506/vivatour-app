/**
 * Tests for request detail error boundary
...
describe('RequestDetailError', () => {
  ...
  describe('Generic Error Handling', () => {
    ...
    it('renders generic error UI for non-not-found errors', () => {
      render(<RequestDetailError error={genericError} reset={mockReset} />);

      expect(screen.getByText('Lỗi tải yêu cầu')).toBeInTheDocument();
      expect(
        screen.getByText('Không thể tải chi tiết yêu cầu. Vui lòng thử lại.')
      ).toBeInTheDocument();
    });
    ...
    it('logs error to console', () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith('[RequestDetailError]', genericError);
    });
  });
  ...
  describe('Not Found Error Handling', () => {
    it('detects "not found" in English error message (lowercase)', () => {
      ...
      render(<RequestDetailError error={notFoundError} reset={mockReset} />);
  ...
  describe('Back Navigation', () => {
    it('renders back button', () => {
      ...
      render(<RequestDetailError error={error} reset={mockReset} />);
  ...
  describe('Layout', () => {
    it('centers content in container', () => {
      ...
      const { container } = render(<RequestDetailError error={error} reset={mockReset} />);
  ...
  describe('Error with Digest', () => {
    it('handles error with digest property', () => {
      ...
      errorWithDigest.digest = 'request-detail-digest123';

      render(<RequestDetailError error={errorWithDigest} reset={mockReset} />);

      expect(mockConsoleError).toHaveBeenCalledWith('[RequestDetailError]', errorWithDigest);
    });
  });
});
